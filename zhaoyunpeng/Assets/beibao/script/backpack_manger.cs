...
public class backpack_manger : MonoBehaviour {
    ...
    void Update() {
        ...
        Vector2 position;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(GameObject.FindGameObjectWithTag("Backpack").transform as RectTransform, Input.mousePosition, null, out position);
        if (isDrag) {
            DragItemUI.show();
